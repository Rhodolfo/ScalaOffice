...
object auxRead { 
  ...
  val dataStr = (cadenaCol, enteroCol.map(_.toString), realesCol.map(_.toString)).zipped.toList.map(x => ReadStr(x._1,x._2,x._3))
  val dataMix = (cadenaCol, enteroCol, realesCol).zipped.toList.map(x => ReadMix(x._1,x._2,x._3))
